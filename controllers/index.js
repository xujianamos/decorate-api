          ...
          .then(function (result) {
            ...
            result.rows.forEach(v => {
              let obj = {
                ...
                cate: v.cate,
                cateName: v.Cate.name, // 获取联表查询中的cate表中的name
                cover: v.Cate.cover,
                createdAt: dateFormat(v.createdAt, 'yyyy-mm-dd HH:MM:ss')
              }
              list.push(obj)
          ...
          .then(function (result) {
            ...
            if (result) {
              resObj.data = {
                ...
                cate: result.cate,
                cover: result.cover,
                cateName: result.Cate.name, // 获取联表查询中cate表中的name
  ...
  let tasks = {
    ...
    query: [
      ...
      (results, cb) => {
        ...
        // 如果从首页请求，只返回4条数据，否则分页查询
        if (req.query.from === 'index') {
          whereCondition.limit = 1
        } else {
          // 根据前端提交的参数计算SQL语句中需要的offset，即从多少条开始查询
          let offset = req.query.rows * (req.query.page - 1) || 0
          //  根据前端提交的参数计算SQL语句中需要的limit，即查询多少条
